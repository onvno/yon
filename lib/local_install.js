...
function* needInstall(parentDir, childPkg, options) {
  ...
  try {
    if (pkg.name && pkg.version && childPkg.version) {
        // 判断本地文件夹是否和父级package.json需要的版本号相匹配
...
/*
production ? production mode install, default is `false`
targetDir npm registry url, default is `https://registry.npmjs.org`
storeDir
timeout ? default is 60000 ms
console ? console logger instance, default is `console`
env ? postinstall and preinstall scripts custom env.
cacheDir ? /Users/AYA/.npminstall_tarball存放压缩目录,需要确认下目录结构
           if `production` mode enable, `cacheDir` will be disable.
binaryMirrors ? default is `{}`
ignoreScripts ? ignore pre / post install scripts, default is `false`
forbiddenLicenses ? Array
flatten ?
*/

/*
yons install gulp@3.9.1 --save
返回config: { registry: 'https://registry.npm.taobao.org',
  pkgs: [ { name: 'gulp', version: '3.9.1', type: 'version' } ],
  root: '/Users/AYA' }
*/

/*
yons install gulp@3.9.1 -g

global config: { registry: 'https://registry.npm.taobao.org',
  pkgs: [ { name: 'gulp', version: '3.9.1', type: 'version' } ],
  root: '/Users/AYA/Documents/yon_pre',
  production: false,
  targetDir: '/Users/AYA/.nvm/versions/node/v6.5.0/lib',
  binDir: '/Users/AYA/.nvm/versions/node/v6.5.0/bin' }
*/
